...
async fn test_create_proof_basic() {
    ...
    let circuit = client
        .create_circuit(dir_path.to_string_lossy().to_string(), Some(vec!["prove_basic".to_string()]), None)
        .await;
...
async fn test_create_proof_input_modes() {
    ...
    // We use tags to distinguish tests run on the same circuit
    let circuit = client
        .create_circuit(dir_path.to_string_lossy().to_string(), Some(vec!["input_modes".to_string()]), None)
        .await;
    ...
    let (str_result, json_result, owned_result) = tokio::try_join!(
        str_submit,
        json_submit,
        owned_submit
    ).unwrap();
